...
from PyQt5.QtWidgets import QApplication, QMainWindow,\
    QPushButton, QWidget, QFormLayout, QLineEdit, QToolBar, QStatusBar, QVBoxLayout,\
        QTextEdit
import math
...
frame_title = "Binance 3Commas bot- Alpha"
...
from lib_pump_dump import AnotherWindow, AnotherWindowDynamicBTC,\
    AnotherWindowDynamicFS, AnotherWindowDynamic, AnotherWindowConfig
...
class Window(QMainWindow):
    ...
    def _createToolBar(self):
        ...
        # self.tools.addAction('Twitter sentiment analysis', self.show_new_window_nimpl)
        # self.tools.addAction('Google trends analysis', self.show_new_window_nimpl)
        # self.tools.addAction('Trading view signals', self.show_new_window_nimpl)
    ...
    def show_new_window_dynamicBTC(self):
        w1 = AnotherWindowDynamicBTC(self.setDisplayText)
        w1.got_signal.connect(self.postprocessliq)
        w1.show()
    ...
    def show_new_window_dynamic(self):
        w1 = AnotherWindowDynamic(self.api, self.exchange)
        w1.show()
        self.popups.append(w1)
    ...
    def postprocessFS(self, emit_dict):
        ...
        # TODO add list of nearest 5 coins
        ...
        for i in ind_:
            line = coins_[i]+\
                                    " has LOWEST Long/Short ACCOUNT ratio in "+\
                                        timeframe_[0]+" timeframe with "+\
                                        str(trades1[i])
            self.write_to_console(line, to_push=1)
        self.write_to_console("\n", to_push=1)
        ...
        for i in ind_:
            line = coins_[i]+\
                                    " has LOWEST Long/Short POSITION ratio in "+\
                                        timeframe_[0]+" timeframe with "+\
                                        str(trades2[i])
            self.write_to_console(line, to_push=1)
        self.write_to_console("\n", to_push=1)
        ...
        for i in ind_:
            line = coins_[i]+\
                                    " has LOWEST Long/Short GLOBAL ACCOUNT ratio in "+\
                                        timeframe_[0]+" timeframe with "+\
                                        str(trades3[i])
            self.write_to_console(line, to_push=1)
        self.write_to_console("\n", to_push=1)
        ...
        for i in ind_:
            line = coins_[i]+\
                                    " has LOWEST BUY/SELL volume ratio in "+\
                                        timeframe_[0]+" timeframe with "+\
                                        str(trades4[i])
            self.write_to_console(line, to_push=1)
    ...
    def _formLayout(self):
        ...
        # self.btn_config = QPushButton('Configure bot settings (config file)')
        # self.btn_config.clicked.connect(self.read_config)
    ...
    def stop_tp_sockets(self):
        print("Stopping ticker for all coins")
        try:
            for symbol in self._sockets:
                bm = self._sockets[symbol]["socketmanager"]
                key = self._sockets[symbol]["key"]
                bm.stop_socket(key)
                bm.close()
        except:
        ...
        try:
            ...
            if self.is_exchange_market:
                ...
                for s in info['symbols']:        
                    symbol = s['symbol']
                    self.price_precision[symbol] = s["pricePrecision"]
    ...
    def on_pump(self):
        ...
        self.filename_ = "trade_logs_" + c_time1+".txt"
        ...
        try:
            ...
            if (self.price_analysis_mode == "last_price"):
            ...
            #     self.conn_key = self.bm.start_ticker_socket_allliq(self.process_message_liq)
                ...
                self.write_to_console("Not yet implemented error, select last_price for price analysis in config!", to_push=1)
            self.bm.start()
            time.sleep(.01)
          ...
          while merge_runningv1:
              ...
              if (coin_trade) and (entry_price > 0.0):
                  ...
                  for i in range(self.lim_trades_per_coin[alt_coin]):
                      ...
                      try:
                          _ = self.api.futures_create_order(symbol=alt_coin, type="LIMIT", quantity=units, \
                                                              positionSide="BOTH", side=type_,\
                                                                  timeInForce="GTC", price=entry_price1)
                          tab_cnt = tab_cnt + 1
                      except:
    ...
    def _binance_buy_sell(self, alt_coin='BTCUSDT', current_value=0.0, \
                          statement=None, indicator_=None):
        leverage = self.leverage
        ...
        bm = BinanceSocketManager(self.api)
        conn = bm.start_symbol_mark_price_socket(symbol=alt_coin, callback=self.sell_trailing_profit, fast=True)
        self._sockets[alt_coin] = {"symbol": alt_coin, "socketmanager": bm, "key": conn}
        bm.start()
        time.sleep(.01)
    ...
    def sell_trailing_profit(self, msg):
        ttp_activated_sell = False
        ttp_activated_buy = False
        
        symbol = msg["data"]['s']
        price = float(msg["data"]['p']) ## market price
        
        if self.trades_completed[symbol]["type_"] == "SELL":
            if price > self.trades_completed[symbol]["sell_value"]:
                ttp_activated_sell = True

        elif self.trades_completed[symbol]["type_"] == "BUY":
            if price < self.trades_completed[symbol]["sell_value"]:
                ttp_activated_buy = True
        
        sold = False
        count = 0
        
        while ttp_activated_sell:
            if count == 0:
                temp  = self.api.futures_position_information()
                entry_price_ = [[float(temp[i1]['entryPrice']),float(temp[i1]['positionAmt'])] \
                                for i1 in range(len(temp)) if temp[i1]['symbol'] == symbol]
                # price_new = entry_price_[0][0]
                units_total = entry_price_[0][1]
            count = 1
            ## place sell order at market
            current_price = float(self.api.futures_symbol_ticker(symbol=symbol)['price'])
            
            if current_price > price*(1 + (self.take_profit_trailing/100.)):
                price = price*(1 + (self.take_profit_trailing/100.))
            
            elif current_price < price:
                _ = self.api.futures_create_order(symbol=symbol, type="MARKET", quantity=units_total, \
                                                      positionSide="BOTH", side="SELL")
                sold = True
                ttp_activated_sell = False
            time.sleep(1)

        while ttp_activated_buy:
            if count == 0:
                temp  = self.api.futures_position_information()
                entry_price_ = [[float(temp[i1]['entryPrice']),float(temp[i1]['positionAmt'])] \
                                for i1 in range(len(temp)) if temp[i1]['symbol'] == symbol]
                units_total = abs(entry_price_[0][1])
            count = 1
            ## place sell order at market
            current_price = float(self.api.futures_symbol_ticker(symbol=symbol)['price'])
        ...
        for ijk in range(len(msg)):
            ...
            if x.symbol not in self.new_list:
                if self.mode_analysis == "Automatic":
                    ...
                    temp_ = [0.2 if np.mean(percentT) < 0.2 else np.mean(percentT)]
                        ...
                        if ((ct - stored_currency.time_stamp_period[i]) >= self.candlesP[i]):
                            ...
                            if execute_trade:
                                process_temp = threading.Thread(target=self.print_statement, args=(c_time, stored_currency.symbol, \
                                                                              ...
                                                                              stored_currency.percent_chgsP, indicator_), daemon=True)
                                process_temp.start()
                ...
                if ((ct - stored_currency.time_stamp_reset) > 15000):
